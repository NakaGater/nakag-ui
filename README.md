...
melta-ui流の「人間にもAIにも読めるデザインシステム」。利用側プロジェクトで1回実行:

```bash
npx @nakagater/ui init
```

これで以下が配置され、Claude Code等のAIエージェントがこのデザインシステムを正しく使うようになります:

- **AGENTS.md** — エージェント共通の参照ルール(マーカーブロックで追記、Codex/Cursor等も読む)
- **CLAUDE.md** — `@AGENTS.md` importの1行(内容の二重管理なし)
- **.claude/skills/nakag-ui/** — UIタスクで自動発動するClaude Codeスキル
- **.claude/settings.json** — Write/Edit直後に `nakag-ui check` を実行するPostToolUse hook(違反はClaudeが自己修正)

AI向けドキュメントはパッケージに同梱されており、常にインストール済みバージョンと一致します:

| ファイル | 内容 |
|---|---|
| `dist/llms.txt` | 入口ドキュメント(配線・コンポーネント表・トークン・禁止事項) |
| `dist/ai/components.json` | 全コンポーネントのexports/variant/size/用例(ソースから自動生成) |
| `dist/ai/tokens.json` | 全トークンのlight/dark実値 |
| `dist/ai/slides.json` | スライドクラス一覧 |
| `dist/ai/rules.json` | 禁止ルール12件(regex detector付き) |

手動検査: `npx --no-install nakag-ui check <files...>`(exit 1 = errorあり)。
Web版: https://nakagater.github.io/nakag-ui/llms.txt
